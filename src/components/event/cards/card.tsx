...
const EventCard: React.FC<EventCardProps> = ({
  ...
  const handleMoreClick = () => {
    ...
    if (!expanded) {
      ...
      gsap.to(card, { height: "22rem", duration: 0.5 });
    } else {
      gsap.from(card, { height: "22rem" });
  ...
  return (
    <article
      ref={cardRef}
      className={`event-card flex flex-row gap-5 mb-20 p-8 border-orange-500 border-2 rounded-lg w-[90%] transition-transform transform hover:scale-105 hover:shadow-lg hover:shadow-[#FFA500] h-[22rem]`}
    >
      <div className="image-container">
        <img
          src={img}
          alt={`Event: ${title}`}
          className="event-image w-[12rem] rounded-md "
        />
      </div>
      <div className={`event-details-container w-[80%]`}>
        <header className="event-title text-3xl mb-3 font-bold">{title}</header>
        <div className="event-details">
          <Separator />
          <p className="event-dates text-2xl mb-3">
            {startDate} - {endDate}
          </p>
          <p className="event-description text-justify text-lg">
            {description}
          </p>
          ...
          <div className="event-buttons grid grid-cols-3 gap-4 items-center ">
            ...
            <div className=" col-span-1 col-end-4">
              <Button
                className="register-button px-10 py-7 text-xl mt-4"
